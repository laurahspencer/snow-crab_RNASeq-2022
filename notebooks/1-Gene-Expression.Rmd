...
```{r}
data.frame(sample=names(counts)) %>% left_join(sample.info%>%mutate(sample=paste0("S",sample))) %>% 
    mutate(s2=gsub("severe_","pH 7.5, ", treatment)) %>% mutate(s2=gsub("long","day 88", s2)) %>% 
    mutate(s2=gsub("moderate_","pH 7.8, ", s2)) %>% mutate(s2=gsub("short","hour 8", s2)) %>% 
    mutate(s2=gsub("ambient","pH 8.0, hour 0", s2)) %>% dplyr::select(s2) %>% write_clip()
```
...
```{r}
as.data.frame(diffex_amb_mod_long_filt) %>% rownames_to_column("gene_id") %>% left_join(snow.annot) %>% 
    dplyr::select(-cds,-protein_id,-GO) %>% distinct() %>% 
    filter(grepl("V-type proton ATPase|Sodium/potassium-transporting ATPase",protein)) %>% 
    arrange(padj)

as.data.frame(diffex_amb_sev_long_filt) %>% rownames_to_column("gene_id") %>% left_join(snow.annot) %>% 
    dplyr::select(-cds,-protein_id,-GO) %>% distinct() %>% 
    filter(grepl("V-type proton ATPase|Sodium/potassium-transporting ATPase",protein)) %>% 
    arrange(padj)
```
