...
gene.counts <- counts.annot.tanner %>%
  left_join(
    diffex.mod.short.filt %>%
    as.data.frame() %>% rownames_to_column("gene_id")) %>%
  filter(!is.na(padj), !is.na(spid)) %>%
  select(gene_id, padj, protein_names, everything()) %>% arrange(padj)

# select treatments to display, or 
b <- c("ambient", "moderate_short", "moderate_long", "severe_short", "severe_long") #all treatments 

gene.counts %>% head(n=15) %>% select(gene_id, padj, spid, evalue, protein_names)

# Plot top 15 differentially expressed genes in te selected treatment 
for (i in 1:nrow(gene.counts[1:15,])) { # here I subset the DEG list to only plot 25 genes - can adjust this number! 
a <-  plotCounts(dds.DESeq.treatment, gene=gene.counts$gene_id[i], intgroup=c("treatment"), replaced = TRUE, returnData = TRUE) %>%
...
print(
  ...
  geom_point(size=2.5, position=position_jitter(w = 0.15,h = 0)) +
    ...
    theme_bw() +
    ggtitle(str_wrap(gene.counts$protein_names[i])) +
...
#### <span style="color:yellow3">Top 15 annotated DEGs, MODERATE OA (pH 7.8) LONG Exposure (yellow)</span>
...
#### <span style="color:sienna2">Top 15 annotated DEGs, SEVERE OA (pH 7.5) SHORT Exposure (orange)</span>
