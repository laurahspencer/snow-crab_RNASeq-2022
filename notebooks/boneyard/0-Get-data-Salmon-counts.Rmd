...
"Salmon did the quantification of the transcript level. We want to see which genes are differential expressed, so we need to link the transcript names to the gene names. We will use the blast GFF file for that, and the GenomicFeatures package:"


### Load libraries

```{r message=FALSE, warning=FALSE, results=FALSE}
list.of.packages <- c("tidyverse", "reshape2", "here", "plotly", "tximport", "GenomicFeatures",  "readr", "readxl") #add new libraries here 
...
<!-- ``` -->
<!-- ## Check out transcript count matrix  -->
...
print(paste("Total number of transcripts in dataframe:", prettyNum(nrow(counts), big.mark = ","), sep=" "))
print(paste("Average number of transcripts per sample:", prettyNum(mean(colSums(counts %>% dplyr::select(contains("sample")) != 0)), big.mark = ","), sep=" "))
print(paste("Total counts, all samples:", prettyNum(sum(colSums(counts %>% dplyr::select(contains("sample")))), big.mark = ","), sep=" "))
#print(paste("Counts for", colnames(counts %>% select(contains("sample"))), ":",  prettyNum(colSums(counts %>% select(contains("sample"))), big.mark = ","), sep=" "))
...
keep2 <- rowSums( counts >= 10 ) >= 0.1*62 #identify genes with counts>=10 across at minimum 10% of the samples
