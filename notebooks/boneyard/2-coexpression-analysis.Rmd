---
title: "02-coexpression-analysis"
...
# Lists of DEGs 
load(file = "../results/deseq2/diffex.mod.short.filt")
load(file = "../results/deseq2/diffex.mod.long.filt")
load(file = "../results/deseq2/diffex.sev.short.filt")
load(file = "../results/deseq2/diffex.sev.long.filt")

# Generate list of genes that are DEGs 
degs <- unique(c(rownames(diffex.mod.short.filt),rownames(diffex.mod.long.filt), rownames(diffex.sev.short.filt), rownames(diffex.sev.long.filt)))

# Load in DESeq2 results lists to pull background list of genes 
load(file = "../results/deseq2/res.mod.short")
load(file = "../results/deseq2/res.mod.long")
load(file = "../results/deseq2/res.sev.short")
load(file = "../results/deseq2/res.sev.long")
```
...
#datExpr0 <- counts.trans[,4:5000] #for testing purposes use subset of data
...
```{r}
#softPower = 14; # for bowtie2 aligned to BKC genome 
...
save(dissTOM, file = "../results/wgcna/dissTom-bowtie-rkc")
load(file = "../results/wgcna/dissTom-bowtie-rkc") #load if needed

## dissTom object generated from BKC-aligned via bowtie
#save(dissTOM, file = "../results/wgcna/dissTom-bowtie")
#load(file = "../results/wgcna/dissTom-bowtie")
...
load(file = "../results/wgcna/geneTree")
...
#load(file = "../results/wgcna/dynamicMods")
#load(file = "../results/wgcna/dynamicMods.50")
load(file = "../results/wgcna/dynamicMods.75")
...
dynamicColors = labels2colors(dynamicMods.75)
table(dynamicColors)
...
for (i in 1:length(modules)) {
  geneInfo.mwu.mods[[i]] <- geneInfo.mwu %>%
    mutate(sign = case_when((moduleColor == modules[[i]] & p.GS.pco2 < 0.05) ~ !!as.symbol(modules.cor[i]), 
      TRUE ~ 0)) %>%
    dplyr::select(id,sign) %>% remove_rownames()
  write.csv(geneInfo.mwu.mods[[i]], paste("../results/GO_MWU/WGCNA-module_",modules[[i]],"_GS05.csv", sep = ""), quote = F,row.names = F)
}
...
for (i in 1:length(modules)) {
  geneInfo.mwu.mods[[i]] <- geneInfo.mwu %>%
    mutate(sign = case_when((moduleColor == modules[[i]] & id %in% degs) ~ !!as.symbol(modules.cor[i]), 
      TRUE ~ 0)) %>%
    dplyr::select(id,sign) %>% remove_rownames()
  write.csv(geneInfo.mwu.mods[[i]], paste("../results/GO_MWU/WGCNA-module_",modules[[i]],"_DEGs.csv", sep = ""), quote = F,row.names = F)
}
...
module.functions <- list(
  ...
  'firebrick4'="(c) Carbohydrate metabolism,\nvesicle transport",
...
#pdf(file="results/wgcna/Enriched-BP_WGCNA.pdf", height = 8, width = 4.5)
tiff(file="../results/wgcna/Enriched-BP_WGCNA.tiff", height = 800, width = 425, res = 100)
MEs %>% 
